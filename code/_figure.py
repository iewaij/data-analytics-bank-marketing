...
ax = sns.histplot(
    data=p, x="index", stat="count", hue="year", bins=500, palette="deep", legend=True
)
...
# 2_15_Job+Marital
job_marital_total = (
    bank_mkt[["job", "marital", "y"]].groupby(["job", "marital"]).count().y.unstack()
)
job_marital_true = (
...
job_marital_heatmap = sns.heatmap(data=job_marital_rate, vmin=0, vmax=0.5, annot=True)
...
job_education_true = (
    bank_mkt[["job", "education", "y"]].groupby(["job", "education"]).sum().y.unstack()
)
...
education_marital_rate = education_marital_rate.rename_axis(None, axis=0).rename_axis(
    None, axis=1
)
